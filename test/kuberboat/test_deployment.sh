...
test_deployment_maintain_replica_1() {
    ...
    sleep 5
    if [ $(json_digit_value "$($kubectl describe deployment $test_deployment_name)" ReadyReplicas) != 0 ]
        then return -1
    fi
    sleep 25
    if [ $(json_digit_value "$($kubectl describe deployment $test_deployment_name)" ReadyReplicas) != $test_deployment_replica_original ]
...
test_deployment_maintain_replica_2() {
    ...
    sleep 5
    if [ $(json_digit_value "$($kubectl describe deployment $test_deployment_name)" Replicas) != $test_deployment_replica_changed ]
        then return -1
    fi
    sleep 25
    if [ $(json_digit_value "$($kubectl describe deployment $test_deployment_name)" ReadyReplicas) != $test_deployment_replica_changed ]
        then return -1
    fi
}
