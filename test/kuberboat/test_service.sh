...
test_container_name="nginx"         # pod_nginx_1.yaml & pod_nginx_2.yaml
...
test_create_service() {
    ...
    sleep 25
    if [ $(json_string_value "$($kubectl describe pod $test_pod_1_name)" Phase) != "Ready" ]
        then return -1
    fi
    if [ $(json_string_value "$($kubectl describe pod $test_pod_2_name)" Phase) != "Ready" ]
        then return -1
    fi
